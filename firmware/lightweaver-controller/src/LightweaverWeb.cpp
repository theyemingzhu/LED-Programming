...
void handleRoot() {
  ...
  page += F(
            ...
            "const blackoutControl=makeConfirmedControl({initial:blackoutOn,description:'change blackout',render:value=>{blackoutOn=value;$('off-btn').classList.toggle('on',value);$('off-btn').textContent=value?'Lights on':'Lights off';$('off-btn').setAttribute('aria-pressed',value?'true':'false')},setDisabled:on=>{$('off-btn').disabled=on},send:value=>controlPost({blackout:value})});"
