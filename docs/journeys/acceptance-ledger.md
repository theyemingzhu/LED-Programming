...
## Phase F checkpoint (director, 2026-09-07, revision `8a60b7a0` + hardening fixtures)

Same 30-suite Chromium set → `{"expected":458,"skipped":5,"unexpected":3}`; the
three were studio-hardening Settings-install cases whose stub seeded a card
already holding the exact project, so the new redundant-install gate truthfully
sent nothing — re-seeded stale, then studio-hardening + journey-ownership +
layout-send-to-card + card-workspace + journey-edits → `{"expected":126,"unexpected":0}`.
Windowless config: one navigation-timing miss in the batch, `{"expected":1,"unexpected":0}`
in isolation. Unit 2369/2369; production build. No hardware, deploy or push.
