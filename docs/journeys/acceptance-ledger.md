...
| J03 | Preserving update of a configured card: capture identity/config, update, reboot, verify running build, return to original task | `tests/preserving-firmware-update.spec.ts:112,178,189,195` | Simulated | not run (this session) | Existing suite; not executed here. |
| J04 | Update transport/preflight variants (current, optional, required, unsupported USB, unavailable release, blank, damaged storage) | `tests/install-update-plan.spec.ts` (all rows); `tests/connection-center-quality.spec.ts:310,338,358,381,416,435,993,1023` | Simulated | not run (this session) | Existing suite; not executed here. |
