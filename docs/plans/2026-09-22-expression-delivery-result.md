...
- Preparation requires exact connected-card identity, ready status, known-good wiring with no pending candidate, compiled-wiring send readiness, commissioning evidence for hardware-affecting installs, and reported capacity when the card supplies it.
- `runExpressionSceneDelivery` saves through the existing owner-capability source path, proves exact source readback, syncs the runtime, and then requires exact post-save card evidence before returning `on-card`.
