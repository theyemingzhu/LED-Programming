      ...
      {wiringCandidate && (
        <section className="lw-wiring-candidate" aria-label="Wiring safety check">
          ...
          {wiringTestState === 'staged' || wiringTestState === 'failed' ? (
            <div><button className="btn primary" data-testid="wiring-test-start" title="Restart the card using the staged wiring so you can check the real LEDs before committing it." data-tooltip="Restart the card using the staged wiring so you can check the real LEDs before committing it." onClick={startWiringTest}>Start light test</button><button className="btn" data-testid="wiring-test-cancel" title="Discard the staged wiring change and keep the card's last working setup." data-tooltip="Discard the staged wiring change and keep the card's last working setup." onClick={() => finishWiringTest(false)}>Cancel change</button></div>
          ) : wiringTestState === 'testing' ? (
            <div><button className="btn primary" data-testid="wiring-test-confirm" title="Confirm the real light test passed and make this wiring the card's working setup." data-tooltip="Confirm the real light test passed and make this wiring the card's working setup." onClick={() => finishWiringTest(true)}>The lights look correct</button><button className="btn" data-testid="wiring-test-reject" title="Reject the tested wiring and restore the card's last working setup." data-tooltip="Reject the tested wiring and restore the card's last working setup." onClick={() => finishWiringTest(false)}>No, restore working setup</button></div>
          ) : <p>Working…</p>}
        </section>
      )}
    </div>
